...
import { RegisterRoomUseCase } from "./register-room-use-case";
...
let sut: RegisterRoomUseCase

describe("Register Room", () => {
    beforeEach(() => {
        floorsRepository = new InMemoryFloorsRepository()
        roomsRepository = new InMemoryRoomsRepository()
        sut = new RegisterRoomUseCase(roomsRepository, floorsRepository)
    })

    test("It should be able to register a room", async () => {
    ...
    test("It should not be able to register a room in an unexistent floor", async () => {
